...
public final class CountryPostgresqlDAO extends SqlConnection implements CountryDAO{
    ...
    public List<CountryEntity> findByFilter(CountryEntity filterEntity) {
        ...
        try (final PreparedStatement preparedStatement = getConnection().prepareStatement(sql.toString())) {
            ...
            throw new NoseException(
                    MessageEnum.COUNTRY_ERROR_FIND_BY_FILTER_SQL.getContent(),
                    MessageEnum.TECHNICAL_ERROR_FIND_BY_FILTER_SQL_COUNTRY.getContent(),
                    exception
            );

        } catch (final Exception exception) {
            throw new NoseException(
                    MessageEnum.COUNTRY_ERROR_FIND_BY_FILTER_UNEXPECTED.getContent(),
                    MessageEnum.TECHNICAL_ERROR_FIND_BY_FILTER_UNEXPECTED_COUNTRY.getContent(),
                    (SQLException) exception
            );

        } catch (final Throwable exception) {
            throw new NoseException(
                    MessageEnum.COUNTRY_ERROR_FIND_BY_FILTER_CRITICAL.getContent(),
                    MessageEnum.TECHNICAL_ERROR_FIND_BY_FILTER_CRITICAL_COUNTRY.getContent(),
                    (SQLException) exception
            );
        }
    }
    ...
    public CountryEntity findById(final UUID id) {
        ...
        try (final PreparedStatement preparedStatement = getConnection().prepareStatement(sql)) {
            ...
            throw new NoseException(
                    MessageEnum.COUNTRY_ERROR_FIND_BY_ID_SQL.getContent(),
                    MessageEnum.TECHNICAL_ERROR_FIND_BY_ID_SQL_COUNTRY.getContent(),
                    (SQLException) exception
            );

        } catch (final Exception exception) {
            throw new NoseException(
                    MessageEnum.COUNTRY_ERROR_FIND_BY_ID_UNEXPECTED.getContent(),
                    MessageEnum.TECHNICAL_ERROR_FIND_BY_ID_UNEXPECTED_COUNTRY.getContent(),
                    (SQLException) exception
            );

        } catch (final Throwable exception) {
            throw new NoseException(
                    MessageEnum.COUNTRY_ERROR_FIND_BY_ID_CRITICAL.getContent(),
                    MessageEnum.TECHNICAL_ERROR_FIND_BY_ID_CRITICAL_COUNTRY.getContent(),
                    (SQLException) exception
            );
        }
    }
